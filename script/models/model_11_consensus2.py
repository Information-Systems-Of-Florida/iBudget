...
class Model11Consensus(BaseiBudgetModel):
    ...
    def _fit_lasso(self, X: np.ndarray, y: np.ndarray) -> None:
        ...
        if self.lasso_alpha is None:
            ...
            lasso_cv = LassoCV(alphas=alphas, cv=5, random_state=self.random_seed,
        ...
        max_observed = np.max(self.weights)
        if max_observed > self.max_weight:
            self.logger.warning(f"Max weight {max_observed:.4f} exceeds limit {self.max_weight}")
    ...
    def _calculate_contributions(self, X: np.ndarray, y: np.ndarray) -> None:
        """Calculate marginal contribution of each model"""
        self.logger.info("")
    ...
    def perform_cross_validation(self, n_splits: int = 10) -> Dict[str, Any]:
        ...
        for fold_idx, (train_idx, val_idx) in enumerate(kf.split(self.X_train), 1):
            ...
            if self.method == 'CLS':
                n_models = X_fold_train.shape[1]
                def objective(w): return np.sum((y_fold_train - X_fold_train @ w)**2)
                constraints = [{'type': 'eq', 'fun': lambda w: np.sum(w) - 1}]
                bounds = [(0, self.max_weight)] * n_models
                w0 = np.ones(n_models) / n_models
                result = minimize(objective, w0, method='SLSQP', bounds=bounds,
                                constraints=constraints, options={'ftol': 1e-9})
                fold_weights.append(result.x)
                ...
                if raw_weights.sum() > 0:
                    ...
                    fold_weights.append(np.ones(n_models) / n_models)
