...
class Model8Bayesian(BaseiBudgetModel):
    ...
    def perform_cross_validation(self, n_splits: int = 10) -> Dict[str, Any]:
        ...
        for fold, (train_idx, val_idx) in enumerate(kfold.split(self.X_train), 1):
            ...
            logger.info(f"Fold {fold}: R^2 = {fold_r2:.4f}")
        ...
        logger.info(f"Cross-validation R^2: {results['cv_r2_mean']:.4f} +- {results['cv_r2_std']:.4f}")
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"  Test R^2: {metrics.get('r2_test', 0):.4f}")
        ...
        print(f"\nCross-Validation:")
        print(f"  CV R^2: {metrics.get('cv_r2_mean', 0):.4f} +- {metrics.get('cv_r2_std', 0):.4f}")
