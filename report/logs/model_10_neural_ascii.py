...
class Model10NeuralNet(BaseiBudgetModel):
    ...
    def run_complete_pipeline(self, fiscal_year_start: int = 2024, fiscal_year_end: int = 2024,
        ...
        if perform_cv:
            ...
            logger.info(f"CV Results: R^2 = {self.metrics['cv_mean']:.4f} +- {self.metrics['cv_std']:.4f}")
    ...
    def perform_cross_validation(self, n_splits: int = 10) -> Dict[str, float]:
        ...
        for fold, (train_idx, val_idx) in enumerate(kf.split(self.X_train), 1):
            ...
            logger.info(f"  Fold {fold}: R^2 = {score:.4f}")
...
def main():
    ...
    print("="*80)
    print(f"\n Performance:")
    print(f"   Test R^2: {results['metrics'].get('test_r2', 0):.4f}")
