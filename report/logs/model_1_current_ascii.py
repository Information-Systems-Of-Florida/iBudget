...
class Model1Linear(BaseiBudgetModel):
    ...
    def plot_diagnostics(self) -> None:
        ...
        if self.outlier_mask is not None and self.y_train is not None:
            ...
            # 3. Model 5b comparison: R^2 over time
            ax = axes[1, 0]
            years = [2015, 2024]
...
def main():
    ...
    print(f"  {'-'*75}")
    print(f"  {'R^2 Score':<20} {0.7998:<20.4f} {model.metrics.get('r2_test', 0):<20.4f} "
    ...
    print(f"  ? 10-Fold CV R^2: {model.metrics.get('cv_mean', 0):.4f} +- {model.metrics.get('cv_std', 0):.4f}")
