...
class Model5Ridge(BaseiBudgetModel):
    ...
    def plot_regularization_path(self):
        ...
        ax.axvline(np.log10(self.optimal_alpha), color='red', linestyle='--', 
                   linewidth=2, label=f'Optimal ? = {self.optimal_alpha:.4f}')
        
        ax.set_xlabel('log??(?)', fontsize=12)
    ...
    def plot_cv_curve(self):
        ...
        ax.plot(np.log10(self.alphas), cv_scores_mean, 'b-', linewidth=2, label='Mean CV R^2')
        ...
        ax.plot(np.log10(self.optimal_alpha), cv_scores_mean[optimal_idx], 
               'ro', markersize=10, label=f'Max R^2 = {cv_scores_mean[optimal_idx]:.4f}')
        
        ax.set_xlabel('log??(?)', fontsize=12)
        ax.set_ylabel('Cross-Validation R^2', fontsize=12)
    ...
    def plot_coefficient_shrinkage(self):
        ...
        ax.set_title(f'Coefficient Shrinkage: OLS vs Ridge (? = {self.optimal_alpha:.4f})', 
    ...
    def perform_cross_validation(self, n_splits: int = 10) -> Dict[str, Any]:
        ...
        Returns:
            Dictionary with CV results (R^2 on original dollar scale)
        """
